...
int dfs(int s, int c_count, int father, int lastT, int tcount, char product[n][n][2], int fatherlastT)
{
    ...
    if(product[c_i][c_j][0] == 'T')
    {
        lastT = s;
        fatherlastT = father;
    }
    if(lastT == prelastT  && product[c_i][c_j][0] == 'T')
    {
        tcount++;
    }
    ...
    if(isComp[t_id])
    {
        ...
        if(tcount == 0)
        {
            printGraphNode(s,product);
            printf("\n---------##C1##--------\n");
            dfs(lastT, c_count + 1, fatherlastT, lastT, tcount + 1, product, fatherlastT);
        }
        if(tcount == 2)
        {
            tcount = 0;
    ...
    for(int x = 0; x < adj[s][SIZE]; x++)
    {
        ...
        if(father != -1 && idInTable(father) == table_id)
        {
            continue;
        }
        printGraphNode(s, product);
        ...
        printf("\n-----------------------\n");
        int prec_count = c_count;

        int bro1 = id(n_i, n_j, intTo_DirectionConverter(0)),
            bro2 = id(n_i, n_j, intTo_DirectionConverter(1)),
            bro3 = id(n_i, n_j, intTo_DirectionConverter(2)),
            bro4 = id(n_i, n_j, intTo_DirectionConverter(3));

        int visbros = (visited[bro1] || visited[bro2] || visited[bro3] || visited[bro4]); 

        if(!visbros)
        {
           c_count = dfs(n_id, c_count, s, lastT, tcount, product, fatherlastT);
        }


        if((x == adj[s][SIZE] - 1) && (prec_count == c_count))
        {
            for(int i = 0; i < adj[s][SIZE]; i++)
            {
                if(idInTable(father) != idInTable(adj[s][i]))
                {
...
void solve(char product[n][n][2]){
    ...
    {
        rem = n * n;
    int C = dfs(id(n/2, n/2, intTo_DirectionConverter(i)), 0, -1, -222, 0, product, -23423);
    printVis();
    if(rem == 0)
        break;
    empty();
