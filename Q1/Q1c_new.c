...
int dfs(int s, char product[n][n][2]){
    ...
    for(int x = 0; x < adj[s][SIZE]; x++){
        ...
        if(!visited[table_id]){
            ...
            // if(rout[directionTo_IntConverter(pos)]){
            //     if(!foundpath[directionTo_IntConverter(pos)]){
            //         if(dfs(n_id, product))  
            //             foundpath[directionTo_IntConverter(pos)] = 1;
            //         else
            //             paste();
            //     }
            // }
...
void addPossibleEdges(int i, int j, int neighbor, char product[n][n][2]){
    ...
    for(int rotThis = 0; rotThis < 4; rotThis++)
    {
        ...
        {
            addEdge(
                    id(i,  j,  intTo_DirectionConverter(rotThis)),
                    id(ni, nj, intTo_DirectionConverter(rotThat))
                );
        }
